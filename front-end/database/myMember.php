<?php
  ...
  class myMember extends memberLog{
    ...
    function signUp(){
      ...
      //비밀번호 암호화
      $userPw = sha1('extrapassword'.$userPw);
      ...
      //기본 프로필 사진 주소 설정
      $profilePhoto = '';
      $profilePhoto = '/CatDogClassifier/front-end/images/logo.png';

      //회원가입시간
      $regTime = time();

      //데이터베이스에 입력
      $sql = "INSERT INTO mymember(userName, email, pw, profilePhoto, regtime) ";
      $sql .= "VALUES('{$userName}','{$userEmail}','{$userPw}','{$profilePhoto}','{$regTime}')";
      ...
      if($res){
        ...
        //회원번호
        $myLog['studentID'] = $this->dbConnection->insert_id;
        //로그 정보 전달
        ...
        $_SESSION['myMemberSes']['studentID'] = $this->dbConnection->insert_id;
    ...
    function photoSave(){
      ...
      $makingFileName = $uploadPhotoFolder.$_SESSION['myMemberSes']['studentID']."."."{$extension}";
      ...
      //사진 정보 업데이트할 회원 번호
      $studentID = $_SESSION['myMemberSes']['studentID'];

      //이미 있는 레코드에 포토 정보를 적용하므로 UPDATE문 사용
      $sql = "UPDATE mymember SET {$photoField} = '{$filePath}' WHERE studentID = {$studentID}";
      $res = $this->dbConnection->query($sql);
